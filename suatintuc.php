...
<body>
	...
	if (isset($_POST["btn_submit"])) {
		...
		if ($tieude == "" || $noidung == "") {
			...
			$sql = "UPDATE tintuc SET tieude = '$tieude',noidung='$noidung',ngaygio='$ngaygio' WHERE id = '$_GET[id]'";

			mysqli_query($conn,$sql);
			?>
			<script type="text/javascript">
				alert('Sửa tin tức thành công');
				window.location.href ='index.php?page=tintuc&&tieude=tintuc';
			</script>
			<?php
		}
	}
		...
		<form class="form-horizontal" role="form" method="post">
			<div class="form-group">
				...
				<div class="col-sm-10 wow bounceInRight" data-wow-duration="1s" data-wow-delay="1s" data-wow-iteration="1">
					<input type="textbox" name='tieude' class="form-control" id="inputEmail3" value="<?php echo $row['tieude'] ?>">
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10 wow bounceInRight" data-wow-duration="1s" data-wow-delay="2s" data-wow-iteration="1">
					<textarea class="form-control" name='noidung' rows="3" ><?php echo $row['noidung'] ?></textarea>
				</div>
			</div>
			<div class="form-group">
